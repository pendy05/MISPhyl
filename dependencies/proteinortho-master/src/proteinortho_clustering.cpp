...
template<class T> void dssyevr_( char* jobz, char* range, char* uplo, int* n, T* a,
	...
	dsyevr_(jobz, range, uplo, n, a,
            ...
            lwork, iwork, liwork, info );
#else
	cerr << string("Error: invalid floattype (should be either float=1 or double=2)!").c_str() << "\n";throw;
#endif
}
...
void printHelp() {
	...
	cerr << "         -conn float      threshold for connectivity ["<<param_con_threshold<<"]" << "\n";
	cerr << "         -purity float    threshold for purity ["<<param_sep_purity<<"]" << "\n";
	cerr << "         -maxnodes int     max. number of nodes for alg. clustering ["<<param_max_nodes<<"]" << "\n";
	cerr << "         -minspecies float threshold for species number ["<<param_min_species<<"]" << "\n";
	cerr << "         -rmgraph STRING   output file for graph" << "\n";
	...
	cerr << "         -powLapD | -power_d float	    the minimum graph density for the power iteration method, lapacks (d|s)syevr is used otherwise [linear:d<10^(-5.2)*n]" << "\n";
	...
	try {
		...
		for (paras = 1; paras < argc; paras++) {
			...
			if (parameter.substr(0, 1) != "-") {
				...
				paras++;
				#ifdef _OPENMP
					if(int(string2floattype(string(argv[paras])))<0){cerr << string("Error: invalid value '"+string(argv[paras])+" for argument "+parameter+"'!").c_str() << "\n";throw;}
					omp_set_dynamic(0);     // Explicitly disable dynamic teams
				...
				cerr << "- test splitGroups() : " << test__splitGroups_result << "\n";
...
/*
		...
		#ifdef timeAnalysis
			for(map<string,floattype>::iterator it = t_master.begin() ; it != t_master.end() ; ++it) cerr << (*it).first << " " << (*it).second << "\n";
		#endif
		#ifdef DEBUG
...
void partition_graph() {
	...
	while( !allNodesAreDone ){
			...
			for (unsigned int protein_id = 0 ; protein_id < graph.size() ; protein_id++) {
				...
				if (cur_cc.size() > param_max_nodes || cur_cc.size() > critical_min_nodes) {
					...
					if(!foundSomeCriticalEdge && cur_cc.size() > param_max_nodes){
						...
						cerr << " [WARNING]  Now restarting the partition algorithm." << "\n";
		...
		if (debug_level > 0) cerr << getTime() << " [DEBUG] Found: " << CC.size() << " connected components in total, continue now with the calculation of the algebraic connectivities." << "\n";
		...
		if(param_lapack_power_threshold_d > -1){
			if (debug_level > 0) cerr << getTime() << " [DEBUG]   Performing now the power iteration of all CC with a graph density of less than "<< param_lapack_power_threshold_d << " (if any)..." << "\n";
		}else{
			if (debug_level > 0) cerr << getTime() << " [DEBUG]   Performing now the power iteration of all CC with a graph density of less than 10^-5.2*n (if any)..." << "\n";
		}
		...
		for(unsigned int i = 0 ; i < CC.size() ; i++){
			...
			if(	( ( param_lapack_power_threshold_d>-1 && CC[i].density < param_lapack_power_threshold_d+0.001 ) || ( param_lapack_power_threshold_d==-1 && CC[i].density < (double)0.00000630957*(double)CC[i].size() ) ) || //param_lapack_power_threshold_d
				...
				if (debug_level > 0 && CC[i].density < param_lapack_power_threshold_d) cerr << getTime() << " [DEBUG]   Found large connected component with "<< CC[i].size() << " nodes and a low graph density of "<< CC[i].density <<". Power iteration is used." << "\n";
				else if (debug_level > 0 && !param_useLapack) cerr << getTime() << " [DEBUG]   Power iteration is used since lapack is turned off (-lapack 0)." << "\n";
				else if(predictedMemoryUseLapack > param_maxRam_inKB) cerr << getTime() << " [WARNING]   Found large connected component with "<< CC[i].size() << " nodes that would overflow the given memory constrains (predicted "<< predictedMemoryUseLapack/1e+3 << "MB > "<<param_maxRam_inKB/1e+3<<"MB) if LAPACK is used. Power iteration is used. (This behavior can be adjusted with -ram)" << "\n";
		...
		if (debug_level > 0) cerr << getTime() << " [DEBUG]   Remaining " << CC.size()-power_calls << " connected components are prepared for algebraic connectivity calculation using LAPACK." << "\n";
		...
		if (debug_level > 0) cerr << getTime() << " [DEBUG]   The algebraic connectivity calculation are now done, continue now with the evaluation of the scores." << "\n";

		// evaluate the connectivities -> reiterating
		for(unsigned int i = 0 ; i < CC.size() ; i++){

			if (debug_level > 0) cerr << getTime() << " [DEBUG]     Connectivity was " << connectivities[i] << "\n";
			...
			if (param_con_threshold && connectivities[i] >= 0 && connectivities[i] < param_con_threshold) {
				if (debug_level > 0) cerr << getTime() << " [DEBUG]     Reiterating" << "\n";
			...
			if (debug_level > 0) cerr << getTime() << " [DEBUG]     Print group:" << "\n";
...
void remove_edge_index(const unsigned int node_id, const unsigned int index) {
	...
	if (index >= graph[node_id].edges.size()) {
		stringstream ss;
		cerr << "Called index out of bounds: " << node.full_name << " sized " << graph[node_id].edges.size() << " called " << index << "\n";
		throw;
	}
	...
	if(graph_clean.size()>tid)
		(*graph_clean[tid]) << node.full_name << "\t" << species[node.species_id] << "\t" << graph[node.edges[index].edge].full_name << "\t" << species[graph[node.edges[index].edge].species_id] << "\n";
	
	// ofstream os((ss.str()).c_str(),std::ios_base::app);
	// os << node.full_name << "\t" << species[node.species_id] << "\t" << graph[node.edges[index].edge].full_name << "\t" << species[graph[node.edges[index].edge].species_id] << "\n";
	// os.close();
...
void stats(floattype i, floattype size) {
	...
	if (last_stat * 1.01 < stat) {
		last_stat = stat;
		cerr << "Clustering: " << setprecision (2) << fixed << stat << "%\n";
	}
}
...
vector<floattype> normalize(vector<floattype> x, floattype *length) {
	...
	*length = (floattype)sqrt(sum);
	if (*length == 0) {*length = 0.000000001;cerr << "normalize" << "\n";}// ATTENTION not 0!
...
floattype getConnectivity(vector<unsigned int>& nodes, bool useLapack) {
		...
		if(isComplete){
			...
			if (conn < param_con_threshold) {
				if (param_min_species >=1 ) {
					...
					if (debug_level > 0) cerr << getTime() << " [DEBUG]   Found " << avg << " genes/species on average. User asked for " << param_min_species << "\n";
	...
	if( n < 32768 && useLapack ){
		...
		if( info > 0 ) {
			cerr << " [WARNING] The algorithm (d|s)syevr failed to compute eigenvalues. Continue now with the slow standard approach (power iteration)." << "\n";
		...
		// 	cerr << " [WARNING] Using (d|s)syevr (lapack) would overflow the given amount of memory. Continue now with the slow standard approach (power iteration)." << "\n";
		// }

		if(param_useWeights && !useWeights){
			cerr << " [WARNING] The maximum number of nodes for the weighted algorithm is exeeded. Continue now with the faster unweighted algorithm." << "\n";
		}
		...
		while(++iter < param_max_iter) { 
			...
			if (debug_level > 2 && iter%100 == 0) cerr << getTime() << " [DEBUG L2] Step " << iter << " / " << param_max_iter << " error:" <<  abs(current_length-last_length) << "\n";
	...
	if (connectivity < param_con_threshold) {
		...
		if (param_min_species >=1 ) {
			...
			floattype avg = calc_group(nodes);
			if (debug_level > 0) cerr << getTime() << " [DEBUG]   Found " << avg << " genes/species on average. User asked for at most " << param_min_species << "\n";
...
void splitGroups(vector<floattype>& y, vector<unsigned int>& nodes , bool useLapack){
	...
	if(y.size() != nodes.size() || y.size()==0){
		cerr << string("[CRITICAL ERROR]   y.size() != nodes.size() || y.size()==0.").c_str() << "\n";throw;
	}
	...
	if( param_useKmereHeuristic && ( 
		...
		if (debug_level > 0) cerr << getTime() << " [DEBUG] (kmere-heuristic) The current connected component is so large that the k-mere heuristic can be used. First: Testing if a normal split would result in a good partition (|.|>20%) of the CC."<< "\n";
		...
		if( (groupA.size() == 0 && groupB.size() == 0) ){
			...
			if (debug_level > 0) cerr << getTime() << " [DEBUG] (kmere-heuristic) All nodes are below the purty threshold. Continue without purity." << "\n";
		...
		if((floattype)groupA.size() > (floattype)0.2*(floattype)nodes.size() && 
			...
			if (debug_level > 0) cerr << getTime() << " [DEBUG] (kmere-heuristic) A normal split would result in a good partition (|.|>20%) of the CC, therefore returning now to the normal algorithm (no k-mere heuristic)." << "\n";
